...
<style>
  /* The picker owns one small SoftUI surface. Keep its tokens local so it
     follows the active app theme without leaking styles into other dialogs. */
  .storage-modal {
    position: fixed;
  ...
  .backdrop {
    ...
    height: 100%;
    padding: 0;
    background: color-mix(in srgb, var(--chrome, #1c2825) 38%, transparent);
    border: 0;
  ...
  .storage-dialog header small,
  .intro p,
  .node-copy small,
  .storage-dialog > footer > span { color: var(--sui-muted); }
  .storage-dialog header small,
  .meter-heading small { font-size: calc(9px * var(--text-scale)); font-weight: 750; letter-spacing: .12em; }
  .storage-dialog h2 { margin: 0; font-size: calc(19px * var(--text-scale)); letter-spacing: -.03em; }
  ...
  .intro strong { font-size: calc(13px * var(--text-scale)); }
  ...
  .destinations button:hover:not(:disabled) { color: var(--sui-primary); box-shadow: var(--sui-raised-sm); transform: translateY(-1px); }
  .destinations button.selected,
  .destinations button.selected:hover:not(:disabled) { color: var(--sui-primary); background: var(--sui-bg); box-shadow: var(--sui-inset-sm); transform: translateY(1px); }
  ...
  .node-icon { display: grid; flex: none; width: 34px; height: 34px; color: var(--sui-primary); background: var(--sui-bg); border-radius: 10px; box-shadow: var(--sui-inset-sm); place-items: center; }
  ...
  .node-copy small { font-size: calc(9px * var(--text-scale)); }
  .destinations button > i { margin-left: auto; color: var(--sui-primary); font-style: normal; font-weight: 800; }

  .meter-card { padding: 15px; background: var(--sui-bg); border: 0; border-radius: 16px; box-shadow: var(--sui-inset); }
  .meter-heading { display: flex; align-items: center; justify-content: space-between; gap: 16px; }
  .meter-heading span { display: grid; gap: 4px; }
  .meter-heading strong { font-size: calc(12px * var(--text-scale)); }
  .meter-heading b { font-size: calc(18px * var(--text-scale)); }
  .meter { height: 7px; margin-top: 12px; overflow: hidden; background: color-mix(in srgb, var(--sui-muted) 14%, transparent); border-radius: 999px; }
  .meter i { display: block; height: 100%; border-radius: inherit; transition: width 180ms ease, background 180ms ease; }

  .storage-dialog > footer { display: flex; align-items: center; justify-content: space-between; gap: 14px; padding: 14px 20px; background: var(--sui-bg); border-top: 1px solid color-mix(in srgb, var(--sui-shadow-dark) 22%, transparent); }
  .storage-dialog > footer > span { display: flex; align-items: center; gap: 7px; font-size: calc(9px * var(--text-scale)); }
  ...
  .save { display: flex; align-items: center; gap: 7px; color: #fff; background: var(--sui-primary); border-color: var(--sui-primary) !important; box-shadow: 4px 4px 12px rgb(48 34 68 / 28%) !important; outline: none; -webkit-appearance: none; appearance: none; -webkit-tap-highlight-color: transparent; }
