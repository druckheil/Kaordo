<script lang="ts">
  ...
  const carried = $derived(snapshot.lingvolernando.pet.accessoryArtifactIds.map((id) => LINGVOLERNANDO_ARTIFACTS.find((item) => item.id === id)).filter(Boolean));
...
<section class="companion-room">
  ...
  <div class="companion-layout">
    <div class="habitat" role="presentation" onpointermove={followPointer} onpointerleave={() => { gazeX = 0; gazeY = 0; }}>
      ...
      <div class="speech"><span></span><strong>{answers === 0 ? 'Let’s remember our first German word.' : accuracy >= 70 ? 'Your recall current feels bright today.' : accuracy >= 45 ? 'This is the useful edge where memory grows.' : 'Forgotten words are not failures. They show us the path.'}</strong><small>{snapshot.lingvolernando.pet.mood} mood · {snapshot.lingvolernando.pet.accessoryArtifactIds.length}/3 artifacts carried</small></div>
    </div>
  ...
  <section class="artifact-wardrobe">
    <header><div><span>Artifact wardrobe</span><h3>Choose up to three orbiting companions</h3><p>Carried artifacts are visual. Equip artifacts in the Vault to activate their learning bonuses.</p></div><strong>{snapshot.lingvolernando.pet.accessoryArtifactIds.length}/3 selected</strong></header>
    {#if discovered.length > 0}
      <div class="artifact-strip">
        {#each discovered.slice(0, 16) as artifact}
          {@const selectedIndex = snapshot.lingvolernando.pet.accessoryArtifactIds.indexOf(artifact.id)}
          <button class:active={selectedIndex >= 0} type="button" onclick={() => onArtifact(artifact.id)} title={artifact.name}>
            <ArtifactGlyph {artifact} size={66} level={snapshot.lingvolernando.artifactLevels[artifact.id] ?? 1} />
            <span><strong>{artifact.name}</strong><small>{selectedIndex >= 0 ? `Orbit ${selectedIndex + 1}` : artifact.rarity}</small></span>
          </button>
        {/each}
      </div>
    {:else}
