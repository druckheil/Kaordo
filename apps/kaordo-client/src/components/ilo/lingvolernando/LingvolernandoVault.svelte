          ...
          {#if isDiscovered}
            ...
            <div class="slot-buttons">
              {#each snapshot.lingvolernando.equippedArtifactIds as equippedId, slot}
                {@const isEquipped = equippedId === selectedArtifact.id}
                <button class:active={isEquipped} class:remove={isEquipped} type="button" onclick={() => onEquip(selectedArtifact.id, slot)} aria-label={`${isEquipped ? 'Unequip' : 'Equip'} ${selectedArtifact.name} ${isEquipped ? `from slot ${slot + 1}` : `in slot ${slot + 1}`}`}><span>{slot + 1}</span>{isEquipped ? 'Unequip' : `Equip slot ${slot + 1}`}</button>
              {/each}
            </div>
