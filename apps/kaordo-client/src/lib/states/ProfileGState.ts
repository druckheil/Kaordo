import type { ProfilePointer, ProfileSaveInput, ProfileSnapshot } from '../domain/profile';
...
export class ProfileGState extends GState<ProfileSnapshot> {
  ...
  reset(): void {
    this.#requestId += 1;
    this.#revokeAvatar(this.snapshot.profile?.avatarUrl ?? null);
    this.#pointer = null;
  ...
  async save(
    ...
    try {
      ...
      this.publish({ error: null, phase: 'ready', profile: { ...result.profile, avatarUrl } });
      return true;
    } catch (error) {
  ...
  #replaceAvatar(next: string | null): void {
    const current = this.snapshot.profile?.avatarUrl ?? null;
    if (next !== current) this.#revokeAvatar(current);
  }
