...
function memoryNodoGateway(): NodoGateway {
  return {
    ...
    moveStorage: () => Promise.resolve({ movedItems: 0 }),
    deleteStorageItem: () => Promise.resolve(),
    ...
    listStorageItems: () => Promise.resolve([]),
  ...
  it('loads and manages the signed-in users Nodo hosts', async () => {
    ...
    render(App, {
      ...
      nodoGateway: {
        ...
        commitPublicStorage: () => Promise.resolve(),
        deleteNode: () => Promise.resolve(),
        renameNode: (_nodeId: string, name: string) => Promise.resolve(name),
